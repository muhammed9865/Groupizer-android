...
class SharedGroupsViewModel(private val repository: DashboardRepository) : ViewModel() {
    ...
    suspend fun getGroupById(groupRequest: GroupRequest) =
        repository.getGroupById(_token.value!!, groupRequest)

    suspend fun createAd() =
        repository.createAd(_token.value!!, Ad(_adTitle.value!!, _adDescription.value!!))
